...
export function Modal({ open, onClose, title, children, footer, fullScreen }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-stretch justify-stretch">
      ...
      <div className={fullScreen ? 'relative m-0 w-full h-full' : 'relative w-full max-w-2xl mx-4 my-auto max-h-[calc(100vh-2rem)] flex'}>
        <div className={(fullScreen ? 'h-full' : 'max-h-full') + ' rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-lg flex flex-col overflow-hidden w-full'}>
          ...
          <div className={fullScreen ? 'flex-1 p-0 overflow-hidden min-h-0' : 'flex-1 min-h-0 p-4 overflow-hidden'}>
            {children}
          </div>
          {footer && !fullScreen && (
